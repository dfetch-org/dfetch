...
def test_add_project_with_dependencies_logs_path_and_url():
    """When metadata has dependencies, their path and url fields are logged."""
    reporter = StdoutReporter(_make_manifest())
    project = _make_project()
    scan = LicenseScanResult(was_scanned=False)
    ...
    meta.dependencies = [
        {
            "destination": "ext/dep",
            "remote_url": "https://example.com/dep.git",
            "branch": "main",
            "tag": "",
            "revision": "aabbccdd",
    ...
    field_names = [call[0][0] for call in mock_field.call_args_list]
...
def test_add_project_dependencies_header_logged():
    """When metadata has dependencies, the 'dependencies' header line is logged."""
    reporter = StdoutReporter(_make_manifest())
    project = _make_project()
    scan = LicenseScanResult(was_scanned=False)
    ...
    meta.dependencies = [
        {
            "destination": "ext/lib",
            "remote_url": "https://example.com/lib.git",
            "branch": "",
            "tag": "v1.0",
            "revision": "deadbeef",
            "source_type": "git-submodule",
