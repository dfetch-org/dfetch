...
class GitLocalRepo:
    ...
    def _configure_eol(self, eol: str) -> None:
        ...
        Enables text=auto detection via .git/info/attributes so git treats
        text files consistently, then sets core.eol as the default checkout
        ending.  Deliberately omits eol= from info/attributes so that
        per-file attributes in the remote's .gitattributes (e.g. *.bat
        eol=crlf) take precedence over the superproject's blanket preference.
        """
        if eol not in ("lf", "crlf"):
            raise ValueError(f"Invalid eol value {eol!r}: must be 'lf' or 'crlf'")
        info_dir = Path(".git") / "info"
        info_dir.mkdir(exist_ok=True)
        (info_dir / "attributes").write_text("* text=auto\n", encoding="utf-8")
    ...
    def _renormalize_eol(self) -> None:
        ...
        changed = (
            run_on_cmdline(logger, ["git", "diff", "--cached", "--name-only"])
            .stdout.decode()
            .strip()
        )
        if not changed:
