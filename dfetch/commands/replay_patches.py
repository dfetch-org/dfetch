...
relative to the upstream source, with no manual setup needed.
...
class ReplayPatches(dfetch.commands.command.Command):
    ...
    def __call__(self, args: argparse.Namespace) -> None:
        ...
        if args.interactive and per_project_counts:
            raise RuntimeError("project:N is not supported with --interactive")
...
def _is_safe_patch_path(patch: str) -> bool:
    ...
    neither the interactive stepper nor the non-interactive replay can end up
    out of sync with a patch that ``apply_patches`` silently skips: if it
    skips a patch, the worktree it produces no longer matches what all
    patches applied would restore, so the project must not be reviewed.
    """
    cwd = Path.cwd()
...
def _can_review_project(
    ...
    if superproject.has_local_changes_in_dir(subproject.local_path):
        ...
        return False
    if any(not _is_safe_patch_path(patch) for patch in subproject.patch):
        logger.print_warning_line(
            project_name,
            "skipped - a patch is missing or outside the manifest root",
        )
        return False
    return True
...
def _apply_review(
    ...
    """Run the review session, pausing for non-interactive inspection."""
    if interactive:
...
def _restore_project(
    ...
    """Restore the project to its original state, from HEAD rather than the worktree.

    A worktree that patch application left in the "right" state isn't
    trustworthy on its own: a fetch or patch step can introduce
    environment-dependent drift (e.g. line-ending normalization) that a diff
    of file contents wouldn't necessarily catch. Restoring from HEAD (git) or
    re-fetching and re-applying (SVN) is the only way to be sure the
    restored state matches what was there before review started.
    """
    if git_super is not None:
...
def _review_projects_combined(
    ...
    try:
        ...
        if staged:
            # Re-announce each project name during restoration: the staging
            # phase above already printed and consumed every project's name
            # header, so without this every "restored" line below would run
            # together under the last project header printed, with no way
...
def _step_tui(patches: list[str], local_path: str, project_name: str) -> None:
    ...
    Patches are applied or reversed directly, with no VCS fetch per step.
    """
    total = len(patches)
    current = 0
    screen = Screen()
