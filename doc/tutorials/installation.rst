...
- ``<version>`` shows the software version:
    ...
    - If it is only numbers (e.g. ``0.14.3``), it is an official release.
...
Every dfetch release ships with cryptographic attestations so you can confirm
that what you downloaded was built from the official source and passed its test
suite.  See :doc:`/howto/verify-integrity` for the full verification steps.
