    ...
    if confdir:
        ...
        if seq_str:
            with contextlib.suppress(Exception):
                ...
                out_png = os.path.join(pytm_static, "threat_model_seq.png")
                _fmt, img_bytes = _pw_render(
        ...
        if dfd_str:
            with contextlib.suppress(Exception):
                ...
                os.makedirs(pytm_static, exist_ok=True)
                out_png = os.path.join(pytm_static, "threat_model_dfd.png")
                subprocess.run(
                    ["dot", "-Tpng", "-o", out_png],
                    input=dfd_str.encode(),
                    capture_output=True,
                    check=True,
                )
...
def _render_seq(seq_str: str, seq_img_path: str = "") -> str:
    ...
    if seq_img_path:
        # Pre-rendered PNG is available: embed directly (works in HTML + PDF).
        return (
    ...
    # Fallback: plantweb UML directive for HTML only (PDF excluded to avoid
    # \includegraphics{None} when the render server is unreachable).
    lines = [".. only:: html", "", "   .. uml::", ""]
...
def _render_dfd(dfd_str: str, dfd_img_path: str = "") -> str:
    ...
    if dfd_img_path:
        # Pre-rendered PNG: embed directly (works in HTML + PDF).
        return (
    ...
    # Fallback: graphviz directive for HTML only (PDF excluded to avoid
    # \includegraphics{None} when dot is absent on the build host).
    lines = [".. only:: html", "", "   .. graphviz::", ""]
