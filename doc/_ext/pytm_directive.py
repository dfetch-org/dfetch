...
def _render_dfd(dfd_str: str, dfd_img_path: str = "") -> str:
    ...
    # Use sphinx.ext.graphviz (local dot binary) so node image= attributes that
    # reference local pytm PNG files are resolvable.  The plantweb/plantuml.com
    # path sends content to a remote server that cannot read local paths,
    # causing graphviz to prepend warning text that corrupts the PNG output.
