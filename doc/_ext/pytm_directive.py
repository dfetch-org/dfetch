...
def _render_seq(seq_str: str) -> str:
    ...
    # Wrap in ``.. only:: html`` so the plantweb UML directive is never
    # included in LaTeX/PDF builds (plantweb contacts a remote render
    # server; if unreachable Sphinx emits \includegraphics{None}).
    lines = [".. only:: html", "", "   .. uml::", ""]
...
def _render_dfd(dfd_str: str) -> str:
    ...
    # Wrap in ``.. only:: html`` so sphinx.ext.graphviz is never invoked
    # in LaTeX/PDF builds (dot binary may be absent, producing
    # \includegraphics{None} and breaking the PDF compile).
